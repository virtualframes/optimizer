...
KEYWORDS = {
    "prompt_injection": re.compile(r"(ignore previous|override|system prompt|jailbreak)", re.I),
    "pii_leak": re.compile(r"(ssn|social security|credit card|passport)", re.I),
    "model_failure": re.compile(r"(timeout|rate.?limit|upstream fail|provider fail)", re.I),
}
...
def classify():
    ...
    for rec in _load_jsonl(CTX):
        ...
        if labels:
            risks.append({"ts": rec.get("ts"), "fingerprint": rec.get("fingerprint"), "labels": labels})
    # latency spikes
    for row in _load_availability():
        ...
        if lat >= LATENCY_WARN_MS or not ok:
            risks.append({"ts": time.time(), "fingerprint": f"avail:{row.get('task')}", "labels": ["latency_spike" if lat>=LATENCY_WARN_MS else "failure"]})
    return risks

...
from services.onedrive.client import (
    list_notebooks, list_sections, list_pages, get_page_content
)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            for nb in notebooks:
                ...
                activity.logger.info(f"Found {len(sections)} sections in notebook {notebook_id}.")
                for sec in sections:
                    section_id = sec["id"]
                    await db_adapter.upsert_section(section_id, sec["displayName"], notebook_id)

                    pages = await list_pages(session, section_id)
                    activity.logger.info(f"Found {len(pages)} pages in section {section_id}.")
                    for pg in pages:
                        page_id = pg["id"]
                        ...
                        activity.logger.info(f"Found {len(points)} note points in page {page_id}.")

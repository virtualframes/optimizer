...
class IssueToPRWorkflow:
    ...
    async def run(self, issue_data: dict) -> str:
        ...
        scan_report = await workflow.execute_activity(
            get_scan_report, {"branch": pr["branch"]},
            start_to_close_timeout=timedelta(minutes=5)
        )

        # 3. Audit Anchoring
        audit_anchor = await workflow.execute_activity(
            ensure_audit_anchor, {"pr_number": pr["pr_number"]},
        ...
        if policy_result.get("allow_merge", False):
            ...
            merge_task = await workflow.execute_activity(
                dispatch_jules_merge, {"pr_number": pr["pr_number"]},
            ...
            reasons = policy_result.get("deny_reason", ["Policy denied merge for an unspecified reason."])
            reasons_str = '; '.join(reasons)
            workflow.logger.warning(f"Merge blocked by OPA policy. Reasons: {reasons_str}")

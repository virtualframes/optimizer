...
async def generate_embeddings(items: list[dict]) -> list[dict]:
    ...
    activity.logger.info(
        f"Generating embeddings for {len(items)} items using {EMBEDDING_MODEL}."
    )
    ...
    response = await client.embeddings.create(
        model=EMBEDDING_MODEL, input=texts_to_embed, dimensions=EMBEDDING_DIM
    )
...
async def index_to_milvus(source: str, embeddings: list[dict]) -> int:
    ...
    activity.logger.info(
        f"Connecting to Milvus at {MILVUS_HOST} to index {len(embeddings)} embeddings for source '{source}'."
    )
    ...
    activity.logger.info(
        f"Inserting {len(embeddings)} vectors into '{collection_name}'."
    )
...
def _create_collection(name: str):
    ...
    activity.logger.info(
        f"Index 'IVF_SQ8' created for collection '{name}'. Loading collection into memory."
    )
    collection.load()
    return collection
...
async def fetch_note_points(since_timestamp: str) -> list[dict]:
    ...
    try:
        ...
        activity.logger.error(
            f"Database error in fetch_note_points: {e}", exc_info=True
        )
        raise
    finally:
        conn.close()
